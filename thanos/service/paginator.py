...
class Paginator:
    def __init__(self, obj_list, page_num, base_url, show_obj_cnt, show_ele_count=11):
    ...
    def pager_html(self):
        ...
        # 上一页
        if self.page_num != 1:
            previous_page = '<li><a href="%s?page=%s">上一页</a></li>' % (self.base_url, self.page_num - 1)
        ...
        for i in range(start_page, end_page + 1):
            if i == self.page_num:
                ...
                ele_li = '<li><a href="%s?page=%s">%s</a></li>' % (self.base_url, i, i)
            ele_list.append(ele_li)

        # 下一页
        if self.page_num != max_page_num:
            next_page = '<li><a href="%s?page=%s">下一页</a></li>' % (self.base_url, self.page_num + 1)
        ...
        if self.page_num == max_page_num:
            ...
            last_page = '<li><a href="%s?page=%s">尾页</a></li>' % (self.base_url, max_page_num)
